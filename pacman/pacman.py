...
# Avoid TensorFlow reserving all GPU memory upfront (harmless if there is
# no GPU: list_physical_devices('GPU') is just empty).
...
def runGames(layout, pacman, ghosts, display, numGames, record, attemp, inicio, final, numTraining=0,
    ...
    if pacman.n2 != 0:
        t = int(time.time())
        nombre_archivo = "data/prob_task_{}_attempt_{}_transfer_from_{}_iter_{}_time_{}.txt".format(pacman.task,attemp,pacman.n1,numTraining,t)
        nombre_archivo_score="data/score_task_{}_attempt_{}_transfer_from_{}_iter_{}_time_{}.txt".format(pacman.task,attemp,pacman.n1,numTraining,t)
        ...
        nombre_archivo = "data/prob_task_{}_attempt_{}_iter_{}_time_{}.txt".format(pacman.task,attemp,numTraining,t)
        nombre_archivo_score = "data/score_task_{}_attempt_iter_{}_time_{}.txt".format(pacman.task, attemp,pacman.n1,numTraining,t)
    open(nombre_archivo, "w").close()
    open(nombre_archivo_score, "w").close()
    open("data/epsilon_para_{}.txt".format(NAME), "w").close()
    open("data/phi_para_{}.txt".format(NAME), "w").close()
    from qlearningAgents import PacmanQAgent, MODEL_REGISTRY
    ...
    if isinstance(pacman, PacmanQAgent) and numTraining == 0:
        # Pure evaluation run (numTraining=0): load the pretrained
        # checkpoint for this difficulty instead of training from scratch.
        if difficulty not in MODEL_REGISTRY:
            raise ValueError(
                f"No checkpoint registered for difficulty {difficulty} in "
                "MODEL_REGISTRY (qlearningAgents.py).")
        name = MODEL_REGISTRY[difficulty]
    ...
    for i in range(numGames):
        ...
        with open("data/epsilon_para_{}.txt".format(NAME), "a") as f:
                f.write(str(e) + "\n")
        with open("data/phi_para_{}.txt".format(NAME), "a") as f:
            f.write(str(phi) + "\n")
    ...
    if (numGames - numTraining) > 0:
        ...
        f.write('\nAverage Score: %s' % (sum(scores) / float(len(scores))))
        f.write('\nScores:        %s' % ', '.join([str(score) for score in scores]))
