...
#
# Modified for "Human-Like Recall/Association for Transfer Learning in
# Reinforcement Learning" (Duban Torres, Avendano, Lozano). This module
# contains the DQN agent (Policy/QLearningAgent/PacmanQAgent) and implements
# the transfer step described in the paper's Algorithm 1 and Equation 1:
# when the memory unit (an LSTM autoencoder, see memory_unit.py) recognizes
# the current 5x5 neighbourhood as similar to a previously learned task, the
# action is chosen from a phi-weighted blend of the old task's Q-values and
# the new task's Q-values instead of the new task's Q-values alone.
...
HEIGTH = 19
MAX_GUARDAR = 500000
# Mean-squared reconstruction error above which the memory unit's
# autoencoder considers a situation *not* similar to the task it was
# trained on (see memory_unit.py and paper Section 5 / Figure 6).
MEMORY_MSE_THRESHOLD = 0.02

# Filenames (under models/) of the pretrained task checkpoints used as the
# "old" policy Q_old when --transfer is requested. Task 0 and task 1
# checkpoints are the two models linked from the top-level README; task 2
# is only needed if you transfer *from* task 2, and is produced by your own
# training run. Edit/extend this if you save checkpoints under other names.
MODEL_REGISTRY = {
    0: "modelo_imagen_20000_04_01_dif0_1575607728_gamma_0.9_attemp_8",
    1: "modelo_imagen_25000_04_01_dif1_1576737275_attemp_3_gamma0.9",
    2: "modelo_imagen_40000_09_01_dif_2_attemp_4_gamma0.9_transfer_from_1",
}
...
class QLearningAgent(ReinforcementAgent):
    ...
    def __init__(self, **args):
        ...
        phi_version = int(args.pop("phi_version", 2))
        ...
        self.BREAK=False
        self.phi_end = 0.1
        # Paper Section 4 describes two phi schedules. Select with
        # -a phi_version=1 (or 2, the default) on the command line.
        if phi_version == 1:
            # V1: phi starts at 0.8 and decays at a fixed per-episode rate,
            # independent of numTraining (reproduces Figures 7-9).
            self.phi = 0.8
            self.phi_decay = PHI_DECAY
        else:
            # V2 (default): phi starts at 1 and the decay rate is solved so
            # phi reaches phi_end exactly at numTraining episodes
            # (reproduces Figures 10-12).
            self.phi = 1
            # numTraining=0 means pure evaluation/data-collection (self.prueba
            # is forced True from the first episode, see runGames()), so the
            # decay rate is never actually applied; avoid a division by zero.
            self.phi_decay = (
                np.exp((np.log(self.phi_end) - np.log(self.phi)) / int(self.numTraining))
                if self.numTraining > 0 else 1.0
            )
        ...
        self.epsilon=EPS_START
        self.eps_decay = (
            np.exp((np.log(EPS_END) - np.log(EPS_START)) / int(self.numTraining / 2))
            if self.numTraining > 0 else 1.0
        )
        ...
        if "transfer" in args.keys():
            ...
            self.n1 = num_first
            self.n2 = num_second
            if num_first not in MODEL_REGISTRY:
                raise ValueError(
                    f"No checkpoint registered for task {num_first} in MODEL_REGISTRY "
                    "(qlearningAgents.py). Add an entry for it or pick a different "
                    "--transfer source task.")
            name = MODEL_REGISTRY[num_first]
            self.policy_first.load_Model("models/" + name + ".h5")

            self.similarity_function = keras.models.load_model("data/" + args["sim_function"])
    ...
    def getAction(self, state):
        ...
        assert len(self.memory) <= self.memory_length, f"Memory holds more than {self.memory_length:d} elements"
        ...
        if len(self.memory) == self.memory_length:  # and self.num_datos < MAX_GUARDAR:
            if self.num_datos < MAX_GUARDAR and self.prueba:
                # Data-collection mode (self.prueba=True, see runGames()):
                # dump the last `memory_length` 5x5-neighbourhood snapshots
                # around Pacman as one training example for the memory unit
                # (see memory_unit.py, which trains an LSTM autoencoder on
                # these "data/piezas_task_<task>" pickles).
                guardar = []
                ...
                filename = f"data/piezas_task_{self.task:d}"
            ...
            # Memory unit: does the last 4 timesteps of 5x5 neighbourhoods
            # around Pacman look like a situation from the source task?
            # (paper Section 5 / Algorithm 1: MU_L(s_t))
            situacion = np.array(self.memory).reshape(1, self.memory_length, 27)
            if self.similarity_function is not None:
                ...
                if mse <= MEMORY_MSE_THRESHOLD:
                    # Similar to the source task: blend the source policy's
                    # Q-values (Q_old) with the current task's Q-values
                    # (Q_new), weighted by phi (paper Equation 1).
